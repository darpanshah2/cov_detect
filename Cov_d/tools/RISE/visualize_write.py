...
def visualize_write(model,img_dir,visualize_dir,CovidDataLoader):
    ...
    for i, (img, img_name) in enumerate(tqdm(data_loader, total=len(data_loader), desc='Generating visualizations')):
        ...
        tensor_imshow(img[0])
        sal = explanations[i]
        plt.imshow(sal, cmap='coolwarm', alpha=0.6)
        ...
        im_val = img[0]
        tensor_imshow(img[0])
        plt.savefig('./test_write_case.png')
